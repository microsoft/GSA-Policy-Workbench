 ...
 * a live-traffic simulation (spec.md §7).
 *
...
export interface PrivateAccessAppMatch {
   ...
   * destination. Per GSA's documented overlap tie-break (docs/Policy-Research.md
   * §5.2): "the segment defined on the Enterprise App will be given priority
...
export interface PrivateAccessWhatIfResult {
   ...
   * against the Private Access forwarding profile (stage 1 — see
   * docs/WhatIf-Spec.md §3/§5). `undefined` when no forwarding-profile data
...
export function resolvePrivateAccessDestination(
  ...
  // Enterprise App > Quick Access overlap tie-break (docs/Policy-Research.md
  // §5.2) — annotate (never remove) any Quick Access match when an Enterprise
