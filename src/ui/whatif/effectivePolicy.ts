 ...
 *
 * Implements the read-only "What-If / effective policy" resolution described
 * in spec.md §7 ("the effective-policy resolver ... shows rule matches but
 * does not simulate live traffic decisions"). It operates entirely on the
...
export interface EffectiveResult {
   ...
   * (stage 1 — see docs/WhatIf-Spec.md §3/§5). Only set when a destination
...
//
// See docs/WhatIf-Spec.md §3/§5. Before a destination can ever be evaluated by
...
// simulates live client/edge connectivity (spec.md §7 / WhatIf-Spec.md §7).
// ---------------------------------------------------------------------------
 ...
 * requested workload's own forwarding profile was even evaluated — see
 * docs/Policy-Research.md §4.1 ("Entra -> M365 -> Private -> Internet",
 * strict order). `entra` is a heuristic match against a static, non-Graph
 ...
 * always processed first, ahead of M365/Private/Internet (see
 * docs/Policy-Research.md §2.1/§4.1) — but it has no Graph representation at
 ...
 * Default Acquire" precedence (docs/Policy-Research.md §2.2/§6) without
 ...
 * workload precedence **Entra → M365 → Private → Internet**
 * (docs/Policy-Research.md §2.1/§4.1). A destination handled by a
 ...
 * workload-order step that is NOT modeled yet — see docs/WhatIf-Spec.md §5
 * item 5 — because it needs the separately-built Private Access view model,
