 ...
 * app can run with no tenant and no sign-in. See docs/architecture.md §6.4.
 */
...
interface GraphRequestOptions {
   ...
   * (needed for `$filter` on `tags/any(...)` — see spec.md §6.1).
   */
   ...
   * v1.0 (e.g. `/servicePrincipals` tag filter — spec.md §6.1).
   */
 ...
 * user, or policy identifiers; see docs/architecture.md §6.3). Callers normally
 ...
 * never the URL, so the audit log stays templated (architecture.md §6.3).
 */
