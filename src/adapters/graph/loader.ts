 ...
 *
 * See docs/architecture.md §6.2 for the adaptive loading contract.
 */
 ...
 * live Graph omits it (the SPA holds only CA stubs on each profile). See
 * docs/architecture.md §6.4 and spec.md §2.3.
 */
...
export async function loadProfileTree(
  ...
  // See spec.md §6.1–6.5 and architecture.md §6.4.
 ...
 * state, priority, associations, and their linked acquisition rules (spec
 * §6.6; docs/WhatIf-Spec.md §3 for how these feed the What-If client-side
 * evaluation). Uses the already-required `NetworkAccess.Read.All` scope, so
