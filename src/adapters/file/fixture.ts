/**
 * Fixture parsing for file-based data source — see docs/architecture.md §6.4.
 *
 ...
 * policy detail as a top-level sibling of `value` (see docs/architecture.md
 * §6.4). Live Graph does not return this sibling — the SPA only sees CA stubs
 * (id + displayName) on each profile — so the loader degrades gracefully when
 * it is absent (spec.md §2.3).
 */
 ...
 * design decision (see docs/architecture.md §6.4, open decision #1).
 */
